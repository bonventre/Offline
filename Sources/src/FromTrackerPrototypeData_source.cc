...
namespace mu2e {
  struct Config
  {
    ...
    fhicl::Atom<unsigned int> maxEvents{Name("maxEvents"), Comment("Verbosity level; larger means more printout"), 0};
  ...
  void TrackerPrototypeDataDetail::readFile(const std::string& filename, art::FileBlock*& fb) {
    ...
    //digis_ = new mu2e::StrawDigiCollection;
    //digiadcs_ = new mu2e::StrawDigiADCWaveformCollection;
      ...
      art::EventPrincipal*& outE)
  {
    ...
    while (true) {
      ...
      if (adc.size() != 27)
        std::cout << " ADC SIZE IS " << adc.size() << std::endl;
